...
struct lifOpLowering : public OpRewritePattern<snn::lifOp> {
  ...
  LogicalResult matchAndRewrite(snn::lifOp op,
    ...
    Value tauConst =
        rewriter.create<arith::ConstantOp>(loc, rewriter.getF32FloatAttr(tau));
    auto tauTensor =
        rewriter.create<tensor::SplatOp>(loc, voltage.getType(), tauConst);
    Value oneMinusTau = rewriter.create<arith::SubFOp>(
        loc,
        rewriter.create<arith::ConstantOp>(loc, rewriter.getF32FloatAttr(1.0f)),
        tauTensor);
    Value oneMinusTauTensor =
        rewriter.create<tensor::SplatOp>(loc, voltage.getType(), oneMinusTau);
    Value decayedVoltage =
        rewriter.create<arith::MulFOp>(loc, voltage, oneMinusTauTensor);
    Value oneConst =
        rewriter.create<arith::ConstantOp>(loc, rewriter.getF32FloatAttr(1.0f));
    Value oneTensor =
        rewriter.create<tensor::SplatOp>(loc, input.getType(), oneConst);
    Value inputTerm = rewriter.create<arith::MulFOp>(loc, input, oneTensor);
    Value updatedVoltage =
        rewriter.create<arith::AddFOp>(loc, decayedVoltage, inputTerm);
    Value thresholdVal = rewriter.create<arith::ConstantOp>(
        loc, rewriter.getF32FloatAttr(threshold));
    Value thresholdTensor = rewriter.create<tensor::SplatOp>(
        loc, updatedVoltage.getType(), thresholdVal);
    ...
    Value cmp = rewriter.create<arith::CmpFOp>(loc, arith::CmpFPredicate::OGT,
                                               updatedVoltage, thresholdTensor);
    ...
    auto outerForOp = rewriter.create<scf::ForOp>(
        ...
        [&](OpBuilder &builder, Location loc, Value i, ValueRange iterArgs) {
          ...
          auto innerForOp = builder.create<scf::ForOp>(
              ...
              [&](OpBuilder &b, Location loc, Value j,
                  ValueRange innerIterArgs) {
                Value elemCmp =
                    b.create<tensor::ExtractOp>(loc, cmp, ValueRange{i, j});
                auto ifOp = b.create<scf::IfOp>(
                    loc, elemCmp,
                    [&](OpBuilder &b2, Location loc) {
                      Value newTensor = b2.create<tensor::InsertOp>(
                          loc, OneConsValue, innerIterArgs[0],
                          ValueRange{i, j});
                      b2.create<scf::YieldOp>(loc, newTensor);
                    },
                    [&](OpBuilder &b2, Location loc) {
                      Value newTensor = b2.create<tensor::InsertOp>(
                          loc, ZeroConsValue, innerIterArgs[0],
                          ValueRange{i, j});
                      b2.create<scf::YieldOp>(loc, newTensor);
                    });
...
namespace {
class SNNToStdPass
    : public mlir::PassWrapper<SNNToStdPass, OperationPass<ModuleOp>> {
public:
