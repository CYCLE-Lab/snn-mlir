...
struct lifOpConversion : public OpRewritePattern<snn::lifOp> {
  ...
  LogicalResult matchAndRewrite(snn::lifOp op, PatternRewriter &rewriter) const override {
    ...
    Value subtauValue = rewriter.create<arith::ConstantOp>(loc, rewriter.getF32Type(),
                                                           rewriter.getF32FloatAttr(subtau));
    Value tauTensor =
        rewriter.create<tensor::EmptyOp>(loc, cast<ShapedType>(voltageType).getShape(),
    ...
    Value outVoltage =
        rewriter.create<tensor::EmptyOp>(loc, cast<ShapedType>(voltageType).getShape(),
                                         cast<ShapedType>(voltageType).getElementType());
    Value decayedVoltage = rewriter
                               .create<linalg::MulOp>(loc, outVoltage.getType(),
                                                      ValueRange{voltage, tauBroadcast}, outVoltage)
                               .getResult(0);
    Value finalVoltage =
        rewriter.create<tensor::EmptyOp>(loc, cast<ShapedType>(voltageType).getShape(),
                                         cast<ShapedType>(voltageType).getElementType());
    Value addtensor = rewriter
                          .create<linalg::AddOp>(loc, finalVoltage.getType(),
                                                 ValueRange{input, decayedVoltage}, finalVoltage)
                          .getResult(0);
    Value thresholdemp =
        rewriter.create<tensor::EmptyOp>(loc, cast<ShapedType>(addtensor.getType()).getShape(),
                                         cast<ShapedType>(addtensor.getType()).getElementType());
    Value thresholdValue = rewriter.create<arith::ConstantOp>(loc, rewriter.getF32Type(),
                                                              rewriter.getF32FloatAttr(threshold));
    Value thresholdTensor =
        rewriter.create<linalg::FillOp>(loc, thresholdValue, thresholdemp).getResult(0);
    ...
    auto cmpType = RankedTensorType::get(dyn_cast<RankedTensorType>(voltageType).getShape(),
                                         dyn_cast<RankedTensorType>(voltageType).getElementType());
    Value cmpTensoremp =
        rewriter.create<tensor::EmptyOp>(loc, dyn_cast<RankedTensorType>(voltageType).getShape(),
                                         dyn_cast<RankedTensorType>(voltageType).getElementType());

    // build linalg::GenericOp
    auto updataop = rewriter.create<linalg::GenericOp>(
        loc, cmpType, ValueRange{thresholdTensor, addtensor}, cmpTensoremp, indexingMaps,
        iteratorTypes, [&](OpBuilder &b, Location loc, ValueRange args) {
          Value thresholdElem = args[0];
          Value voltageElem = args[1];
          Value comparisonResult =
              b.create<arith::CmpFOp>(loc, arith::CmpFPredicate::OLT, voltageElem, thresholdElem);
...
public:
  ...
  void getDependentDialects(mlir::DialectRegistry &registry) const override {
    registry.insert<mlir::arith::ArithDialect, mlir::scf::SCFDialect, mlir::linalg::LinalgDialect,
                    mlir::tensor::TensorDialect>();
  }

  StringRef getArgument() const final { return "convert-snn-to-linalg"; }

  StringRef getDescription() const final { return "Convert SNN operations to Linalg operations."; }
