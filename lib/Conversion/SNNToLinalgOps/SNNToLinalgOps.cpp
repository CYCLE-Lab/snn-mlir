...
struct lifOpConversion : public OpRewritePattern<snn::lifOp> {
  ...
  LogicalResult matchAndRewrite(snn::lifOp op,
    ...
    Value subtauValue = rewriter.create<arith::ConstantOp>(
        loc, rewriter.getF32Type(), rewriter.getF32FloatAttr(subtau));
    Value tauTensor = rewriter.create<tensor::EmptyOp>(
        loc, cast<ShapedType>(voltageType).getShape(),
        cast<ShapedType>(voltageType).getElementType());
    Value tauBroadcast =
        rewriter.create<linalg::FillOp>(loc, subtauValue, tauTensor)
            .getResult(0);
    Value outVoltage = rewriter.create<tensor::EmptyOp>(
        loc, cast<ShapedType>(voltageType).getShape(),
        cast<ShapedType>(voltageType).getElementType());
    Value decayedVoltage =
        rewriter
            .create<linalg::MulOp>(loc, outVoltage.getType(),
                                   ValueRange{voltage, tauBroadcast},
                                   outVoltage)
            .getResult(0);
