...
struct lifOpLowering : public OpRewritePattern<snn::lifOp> {
  ...
  auto addOp = rewriter.create<linalg::AddOp>(loc, finalVoltage.getType(), ValueRange{input, decayedVoltage}, finalVoltage );

  //构建阈值tensor
  Value thresholdemp = rewriter.create<tensor::EmptyOp>(loc, finalVoltage.getType().cast<ShapedType>().getShape(), finalVoltage.getType().cast<ShapedType>().getElementType());
  ...
  auto updataop = rewriter.create<linalg::GenericOp>(
      loc, cmpType, // 输出类型
      ValueRange{thresholdTensor, finalVoltage}, // 输入张量
      cmpTensoremp, // 输出张量
...
public:
  ...
  StringRef getArgument() const final {
    return "convert-snn-to-Linalgops";  // 命令行标识符
  }
