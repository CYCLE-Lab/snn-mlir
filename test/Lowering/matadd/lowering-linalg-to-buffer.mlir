...
module {
  ...
  func.func @test_matadd(%arg0: tensor<4x5xf32> {onnx.name = "x"}, %arg1: tensor<4x5xf32> {onnx.name = "y"}) -> (tensor<4x5xf32> {onnx.name = "output"}) {
    // CHECK-NEXT: linalg.generic {indexing_maps = [#[[$MAP]], #[[$MAP]], #[[$MAP]]], iterator_types = ["parallel", "parallel"]} ins([[ARG0]], [[ARG1]] : memref<4x5xf32>, memref<4x5xf32>) outs([[ARG2]] : memref<4x5xf32>) {
