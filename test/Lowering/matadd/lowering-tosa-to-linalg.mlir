...
func.func @test_matadd(%arg0: tensor<4x5xf32> {onnx.name = "x"}, %arg1: tensor<4x5xf32> {onnx.name = "y"}) -> (tensor<4x5xf32> {onnx.name = "output"}) {
  // CHECK-NEXT: [[EMPTY:%[0-9a-zA-Z_]*]] = tensor.empty() : tensor<4x5xf32>
  // CHECK-NEXT: [[GENERIC:%[0-9a-zA-Z_]*]] = linalg.generic {indexing_maps = [#[[$MAP]], #[[$MAP]], #[[$MAP]]], iterator_types = ["parallel", "parallel"]} ins([[ARG0]], [[ARG1]] : tensor<4x5xf32>, tensor<4x5xf32>) outs([[EMPTY]] : tensor<4x5xf32>) {
  // CHECK-NEXT: ^bb0([[IN:%[0-9a-zA-Z_]*]]: f32, [[IN0:%[0-9a-zA-Z_]*]]: f32, [[OUT:%[0-9a-zA-Z_]*]]: f32):  
  // CHECK-NEXT:   [[ADD:%[0-9a-zA-Z_]*]] = arith.addf [[IN]], [[IN0]] : f32
  // CHECK-NEXT:   linalg.yield [[ADD]] : f32
  // CHECK-NEXT: } -> tensor<4x5xf32>
  %0 = tosa.add %arg0, %arg1 : (tensor<4x5xf32>, tensor<4x5xf32>) -> tensor<4x5xf32>
  // CHECK-NEXT: return [[GENERIC]] : tensor<4x5xf32>
